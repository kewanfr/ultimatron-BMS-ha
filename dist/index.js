...
function publishBatteryStateHA(battery, state) {
    ...
    client.publish(`homeassistant/sensor/${battery.name}_current/state`, state.current.toString());
...
(async () => {
    const batteries = await battery_1.UltimatronBattery.findAll(60000, 2, false, 10 * 60 * 1000 * 10);
